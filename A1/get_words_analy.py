...
if __name__ == '__main__':
        ...
        if file.endswith('.html'):
            with open(os.path.join(html_dir, file), 'r', encoding='utf-8') as html_file:
                ...
                for word, count in most_common:
                    total_words += count
                with open(os.path.join(PROJECT_NAME, 'vocab_vs_col_' + DOMAIN_NAME + '.csv'), 'a',
                          encoding='utf-8') as f:
                    print(len(most_common), total_words, sep=", ", file = f)
    ...
    # with open(os.path.join(PROJECT_NAME, 'frequentWords_' + DOMAIN_NAME + '.csv'), 'w', encoding='utf-8') as f:
    #     for word, count in most_common:
